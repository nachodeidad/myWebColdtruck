...
import { ChevronLeftIcon, ChevronRightIcon, EditIcon, SearchIcon, UserIcon, UserPlusIcon, Calendar, Clock, Lock } from "lucide-react"
        ...
        <div className="flex gap-6 w-full max-w-[1400px]">
                    ...
                    {selectedUser.phoneNumber && (
                        ...
                        <div className="flex items-center">
                          <span
                            className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${
                              roleStyles[selectedUser.role] || roleStyles.driver
                            }`}
                          >
                            {getRoleLabel(selectedUser.role)}
                          </span>
                      ...
                      <div className="col-span-2 grid grid-cols-subgrid border-t border-t-[#cedbe8] py-5">
                        ...
                        <div className="flex items-center">
                          <span
                            className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${
                              statusStyles[selectedUser.status] || statusStyles.Disabled
                            }`}
                          >
                            {selectedUser.status}
                          </span>
          ...
          <div className="flex flex-col flex-1 min-w-0">
            ...
            <div className="flex flex-wrap justify-between gap-3 p-4">
              <p className="text-[#0d141c] tracking-tight text-[32px] font-bold leading-tight">
                Users
              </p>
              <button
                onClick={() => setShowRegisterModal(!showRegisterModal)}
                className="flex min-w-[84px] max-w-[480px] cursor-pointer items-center justify-center overflow-hidden rounded-lg h-8 px-4 bg-[#e7edf4] text-[#0d141c] text-sm font-medium leading-normal hover:bg-[#d1dae6] transition-colors"
              >
                <span className="truncate">{showRegisterModal ? "Cancel" : "Add User"}</span>
              </button>
            </div>
                  ...
                  <table className="flex-1">
                    ...
                    <tbody>
                      {currentUsers.map((user) => (
                        <tr 
                          key={user.id} 
                          className={`border-t border-t-[#cedbe8] hover:bg-gray-50 transition-colors cursor-pointer ${
                        ...
                        >
                          ...
                          <td className="h-[72px] px-4 py-2 text-[#49739c] text-sm font-normal leading-normal">
                            <span
                              className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${
                                roleStyles[user.role] || roleStyles.driver
                              }`}
                            >
