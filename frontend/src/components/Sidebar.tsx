...
export const Sidebar: React.FC<SidebarProps> = ({ selected, setSelected }) => {
  ...
  const getNavigationOptions = () => {
    ...
    return user.role === "admin" ? [...commonOptions, ...adminOptions] : [...commonOptions, ...driverOptions]
  }
...
const Option: React.FC<OptionProps> = ({ 
  ...
  return (
    ...
    >
      <span
        className={`grid h-6 w-6 place-content-center transition-colors ${
...
const TitleSection: React.FC<TitleSectionProps> = ({ open, user, selected, setSelected }) => {
  ...
  return (
    <motion.button
      ...
      className={`flex items-center gap-3 rounded-xl border p-3 shadow-lg w-full transition-all duration-200 ${
        isSelected
          ? "bg-blue-700 border-blue-600 ring-2 ring-blue-300"
    ...
    >
      <div className="relative">
        <div className="h-10 w-10 overflow-hidden rounded-full bg-gradient-to-br from-blue-500 to-blue-700 ring-2 ring-blue-300">
          {user.profilePicture ? (
            <img src={user.profilePicture || "/placeholder.svg"} alt="Profile" className="h-full w-full object-cover" />
          ) : (
