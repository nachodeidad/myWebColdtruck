...
interface AlertsByTruck {
  truck: TruckType;
  alerts: any[]; // Se acepta estructura simple
}
        ...
        {loading ? (
          ...
          <div className="space-y-8">
            {data.map((item) => {
              ...
              return (
                <div key={truck?._id || Math.random()} className="bg-white rounded-lg shadow-sm border border-gray-200">
                  <div className="px-6 py-4 border-b border-gray-200">
                    <div className="flex items-center gap-3">
                      <Truck className="h-5 w-5 text-blue-600" />
                      <h3 className="font-semibold text-lg">
                        Truck {truck?.plates || truck?._id || "Sin placa"}
                      </h3>
                    </div>
                  ...
                  <ul className="divide-y divide-gray-100">
                    {item.alerts.map((a) => (
                      <li key={a._id} className="px-6 py-4 flex items-start gap-3">
                        <AlertTriangle className="h-5 w-5 text-red-600 mt-1" />
                        <div className="text-sm text-gray-800">
                          <div className="font-medium">
                            {a.type} {/* No hay dateTime */}
                          </div>
                          <div className="text-gray-600">{a.description}</div>
                          {/* Mostramos temperatura/humedad si existen */}
                          {(a.temperature || a.humidity) ? (
                            <div>
                              {a.temperature ? `${a.temperature}°C` : "—"} / {a.humidity ? `${a.humidity}%` : "—"}
                            </div>
                          ) : null}
                        </div>
                      </li>
